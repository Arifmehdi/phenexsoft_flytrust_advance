<div class="bravo_topbar u-header__hide-content u-header__topbar u-header__topbar-lg @if(!empty($is_home)|| !empty($header_transparent)) @else  border-color-8 @endif"> <!--border-bottom border-color-white-->
   <div class="{{$container_class ?? 'container'}}">
       <div class="d-flex align-items-center">
           ...
           <!-- phone_contact -->
            ...
            <!-- admin_email -->
            @if(!empty($admin_email = setting_item("admin_email")))

                   <div class="d-flex align-items-center text-white px-3">
                       <i class="fa fa-envelope mr-2 ml-1 font-size-18"></i>
                       <span class="d-inline-block font-size-14 mr-1">{{   $admin_email }}</span>
                   </div>
            @endif
           <div class="ml-auto d-flex align-items-center">
               ...
               @endif 
               --}}
               <div class="list-inline u-header__topbar-nav-divider mb-0 topbar_left_text text-white font-size-14 @if(!empty($is_home)|| !empty($header_transparent)) @else  list-inline-dark @endif">
               {!! setting_item_with_lang("topbar_left_text") !!}
           </div>
                   ...
                   @else
                       <div class="d-flex align-items-center text-white py-3 dropdown">
                           ...
                           <ul class="dropdown-menu dropdown-menu-user text-left dropdown">
                               ...
                               @endif
                               @if(is_vendor())
                                   <li class="d-none"><a href="{{route('vendor.dashboard')}}" class=""><i class="icon ion-md-analytics"></i> {{__("Vendor Dashboard")}}</a></li>
                               @endif
                               @if(is_admin())
                                   <li class=""><a href="{{url('/admin')}}"><i class="icon ion-ios-ribbon"></i> {{__("Dashboard")}}</a></li>
                               @endif
                               ...
                               @endif
                               <li class="d-none"><a href="{{route('user.booking_history')}}"><i class="fa fa-clock-o"></i> {{__("Booking History")}}</a></li>
